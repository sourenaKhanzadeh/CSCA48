...
class SingleLinkedList():
    ...
    def insert(self, index, item):
        ...
        if (index > self._size - 1):
            ...
            self.add_first(item)
        # otherwise traverese the list to insert it in the right place
        else:
            # start from the head
            cur = self._head
...
class LLNodeII:
    def __init__(self, o, nextn=None):
        self.data = o
        self.next = nextn
...
if __name__ == "__main__":
    node5 = LLNodeII("2")
    node4 = LLNodeII("3", node5)
    node3 = LLNodeII("4", node4)
    node2 = LLNodeII("5", node3)
    node = LLNodeII("6", node2)

    my_data = reverse(node)
    curr = node
    res = ''
